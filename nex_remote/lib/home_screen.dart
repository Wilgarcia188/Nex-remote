...
class _EntryRow extends StatelessWidget {
  ...
  const _EntryRow({required this.entry, required this.onEdit, required this.onDelete});

  IconData get _actionIcon {
    final a = entry.action;
    if (a is OpenAppAction) return Icons.launch;
    if (a is SystemAction) {
      return switch (a.systemAction) {
        'back'       => Icons.arrow_back,
        'home'       => Icons.home_outlined,
        'recents'    => Icons.apps,
        'screenshot' => Icons.crop_free,
        _            => Icons.settings_remote,
      };
    }
    if (a is ScrollAction) return a.direction == 'up' ? Icons.keyboard_arrow_up : Icons.keyboard_arrow_down;
    if (a is ClickAction) return Icons.touch_app;
    if (a is ToggleMouseAction) return Icons.mouse;
  ...
  Widget build(BuildContext context) => ListTile(
        ...
        title: Row(children: [
          Icon(_actionIcon, size: 15, color: Colors.tealAccent.withValues(alpha: 0.8)),
